...
fn natural_key(s: &str) -> Vec<NaturalChunk> {
    ...
    while chars.peek().is_some() {
        if chars.peek().map(|c| c.is_ascii_digit()).unwrap_or(false) {
            ...
            // Parse as u64 — handles up to 18-digit runs without overflow.
            let n: u64 = digits.parse().unwrap_or(u64::MAX);
            chunks.push(NaturalChunk::Number(n));
        } else {
            let text: String = chars.by_ref().take_while(|c| !c.is_ascii_digit()).collect();
...
pub fn scan_folder(path: &Path) -> Vec<FileEntry> {
    ...
    videos.sort_by(|a, b| sort_key(a).cmp(&sort_key(b)));
    subtitles.sort_by(|a, b| sort_key(a).cmp(&sort_key(b)));
...
#[cfg(test)]
mod tests {
    use super::{NaturalChunk, extract_number, natural_key};
